...
def run_cell_filtration_experiment(
    ...
    for pct in filter_percentiles:
        ...
        if n_patients_kept < len(adata.obs["patient_id"].unique()):
            if verbose:
                print(
                    f"    WARNING: Only {n_patients_kept} patients have cells after filtering"
                )
        ...
        try:
            ...
            if verbose:
                print(
                    f"    AUC: {filtered_result['auc']:.4f} (change: {filtered_result['auc'] - baseline_auc:+.4f})"
                )
    ...
    if verbose:
        ...
        for pct, auc in sorted(filtered_aucs.items()):
            ...
            print(
                f"  {pct:3d}% removed: AUC = {auc:.4f} (change: {auc - baseline_auc:+.4f}){marker}"
            )
...
def plot_cell_filtration_results(
    ...
    parts = ax3.violinplot(
        [nr_scores, r_scores], positions=[0, 1], showmeans=True, showmedians=True
    )
...
def run_external_validation(
    ...
    if verbose:
        ...
        print(
            f"BCC data: {adata_bcc.n_obs} cells from {adata_bcc.obs['patient_id'].nunique()} patients"
        )
    ...
    for patient in unique_patients:
        ...
        patient_labels[patient] = int(
            adata_bcc.obs.loc[mask, "response_binary"].iloc[0]
        )
...
def plot_external_validation(
    ...
    ax2.set_title(
        "Patient Signature Scores by Response", fontsize=12, fontweight="bold"
    )
...
def compute_shap_values(
    ...
    try:
        ...
        raise ImportError(
            "SHAP package is not installed. " "Install it with: pip install shap"
        )
...
def plot_shap_analysis(
    ...
    colors = [
        "#f97316" if g in PRECISE_11_GENE_SIGNATURE else "#3b82f6" for g in top_genes
    ]
    ...
    ax1.set_title(
        f"Top {top_n} Genes by SHAP Importance", fontsize=12, fontweight="bold"
    )
    ...
    ax2.scatter(
        xgb_norm[combined_top],
        shap_norm[combined_top],
        alpha=0.6,
        c="#3b82f6",
        edgecolor="white",
        s=50,
    ...
    sig_idx = [
        gene_names.index(g) for g in PRECISE_11_GENE_SIGNATURE if g in gene_names
    ]
    ax2.scatter(
        xgb_norm[sig_idx],
        shap_norm[sig_idx],
...
def run_all_stretch_goals(
    ...
    if verbose:
        ...
        if run_goal_b and results.get("goal_b"):
            print(
                f"✓ Goal B (External Validation): BCC AUC = {results['goal_b']['auc']:.4f}"
            )

        if run_goal_c and results.get("goal_c"):
            print(
                f"✓ Goal C (SHAP): Top gene = {results['goal_c']['top_genes_shap'][0]}"
            )
